...
namespace Pangya_Modern_Editor.Extensions
{
    public class Util
    {
        ...
        public static object Part_gerarSql(List<Part> lista, string Arquivo, ref BackgroundWorker BW)
        {
            ...
            checked
            {
                foreach (Part listum in lista)
                {
                    ...
                    string text7 = Conversions.ToString((int)listum.type_item);
        ...
        public static object getImage(string icon = "", bool Online = false)
        {
            Image image;
            try
            {
                ...
                {
                    image = Image.FromFile(Directory.GetCurrentDirectory() +$@"Icon\{icon}.png");
                }   
                return image;
            }
            catch { 
                return Resources.ajax_loader;
            }
        }
    }
