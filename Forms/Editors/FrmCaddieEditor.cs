...
{
    public partial class FrmCaddieEditor : Form
    {
        public FrmCaddieEditor()
        {
        ...
        private void CarregarItem()
        {
            if (this.ListaItem.SelectedCells[0].RowIndex > -1)
            {
                ...
                if (!this.lsTemp[num2].Level.is_max)
                {
                    cbLevel.SelectedIndex = this.lsTemp[num2].Level.level;
        ...
        {
            this.filtrar();
            this.imgStatus.Image = (this.ComboBox2.SelectedIndex != 0) ? ((this.ComboBox2.SelectedIndex != 1) ? this.ImageList1.Images[2] : this.ImageList1.Images[1]) : this.ImageList1.Images[0];
        }
        ...
        private void salvarAlteracoes()
        {
            ...
            caddie.Active = (uint)(ckAtivo.Checked == true ? 1 : 0);
            ...
            {
                if (caddie.Level.is_max)
                {
                    caddie.Level.is_max = false;
                }
                else
                {
            ...
            {
                caddie.Level.is_max = true;
            }
            if (ckTempoAtivo.Checked)
            {
                caddie.date.active = 1;
                caddie.date.Start.Time = this.dtInicio.Value;
                caddie.date.End.Time = this.dtInicio.Value;
            }
            lsTemp[num] = caddie;
            CarregarItem();
        ...
        private void MenuSalvarBackup_Click(object sender, EventArgs e)
        {
            ...
            bool data = true;
            int totalProc = 0;
            if (Convert.ToBoolean(Util.gerarBackup(bs, MySettingsProperty.Settings.ArquivoIff, data, totalProc)))
            {
        ...
        private void txtIcone_TextChanged(object sender, EventArgs e)
        {
            ...
            this.carregarImagem(Util.getImage(this.txtIcone.Text) as Image, ref imgIcone);
